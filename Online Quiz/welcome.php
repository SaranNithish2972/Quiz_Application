<?php
    ...
    else
    {
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            $_SESSION['name'] = $row['name']; // Assign the 'name' from database to $_SESSION['name']
        } else {
            // Handle case where no user data found
            $_SESSION['name'] = ''; // Set to empty if no name found
        }
    }

    // Now $_SESSION['name'] should be properly initialized
    $name = isset($_SESSION['name']) ? $_SESSION['name'] : ''; // Assign to $name
?>
    ...
    <div class="container">
        <div class="row">
            <div class="col-md-12">
                <?php if(@$_GET['q']==1) 
                {
                    echo '<center><h2>Welcome to Quiz Platform</h2></center>';
                    echo '<center><h3>Welcome, ' . $name . '! Explore and start your quizzes here.</h3></center>';
                    echo '<center><h3>INSTRUCTION:</h3></center>';
                    echo '<center><h3>Answer carefully, you only get a chance.</h3></center>';
                ...
                if (@$_GET['q'] == 'quiz' && @$_GET['step'] == 2) {
                    ...
                    // Fetch Questions
                    $questions_query = mysqli_query($con, "SELECT * FROM questions WHERE eid='$eid' AND sn='$sn'");
                    $question_row = mysqli_fetch_array($questions_query);
                    $qid = $question_row['qid'];
                    $qns = $question_row['qns'];

                    // Initialize variables to determine first and last question
                    $is_first_question = ($sn == 1);
                    $is_last_question = ($sn == $total);

                    echo '<div class="panel" style="margin:5%">';
                    echo '<b>Question &nbsp;' . $sn . '&nbsp;::<br /><br />' . $qns . '</b><br /><br />';

                    // Fetch the options for the current question
                    $options_query = mysqli_query($con, "SELECT * FROM options WHERE qid='$qid'");
                    echo '<form id="quizForm" action="update.php?q=quiz&step=2&eid=' . $eid . '&n=' . $sn . '&t=' . $total . '&qid=' . $qid . '" method="POST"  class="form-horizontal"><br />';
                    while ($row = mysqli_fetch_array($options_query)) {
                        $option = $row['option'];
                    ...
                    if (!$is_last_question) {
                        ...
                        echo '<button type="submit" name="action" value="submit" class="btn btn-primary">Next</button>';
                    } else {
                        // If last question, show "Submit Quiz" button
                        echo '<button type="submit" name="action" value="submit" class="btn btn-primary">Submit Quiz</button>';
                    }
                    ...
                    //     $prev_sn = $sn - 1;
                    //     echo '<button type="button" onclick="navigateTo(' . $prev_sn . ')" class="btn btn-secondary">Previous</button>&nbsp;';
                    // }

                    // // Next Question Button or Submit Quiz Button
                    // if (!$is_last_question) {
                    //     $next_sn = $sn + 1;
                    //     echo '<button type="button" onclick="navigateTo(' . $next_sn . ')" class="btn btn-primary">Next</button>&nbsp;';
                    // } else {
                    //     echo '<button type="submit" class="btn btn-primary">Submit Quiz</button>&nbsp;';
                    // }
                    ...
                    //     }
                    //     document.getElementById("timer").innerHTML = "Time remaining: " + timeout + " seconds";
                    // }, 1000);
                    // </script>';
                    // echo '<div id="timer" style="font-size:20px; color:red;"></div>';
                }
...
<script>
function navigateTo(newSn) {
    var eid = '<?php echo $eid; ?>';
    var total = '<?php echo $total; ?>';
    var url = "welcome.php?q=quiz&step=2&eid=" + eid + "&n=" + newSn + "&t=" + total;
    console.log("Navigating to:", url); // Debugging line to check URL
